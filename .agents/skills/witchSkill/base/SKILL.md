...
| [doLua](./dolua/SKILL.md) | `doLua` | xLua 反射逃生舱 — 任意 Lua + 原生 `CS` 访问任意已加载 C# 类型（含其他 mod 的私有成员） |
...
| [Diagnostics](./diagnostics/SKILL.md) | `inspect`, `query_config`, `search_config`, `dump_mod_state`, `get_scene_tree`, `get_recent_logs`, `raycast_mouse`, `set_rng_seed`, `get_screenshot`, `give_item`, `scan_ui`, `click_ui` | Developer backdoor tools — reflection, config queries, scene/UI inspection, item injection |
