...
> ⚠️ **所有列出的工具均由基座 `WitchModMCP.Contracts` 实现**（无独立 DeveloperTools Mod），与 `base/` 模块是同一批工具、不同的"开发调试"组织视角。
...
| 工作流 | 步骤 | 涉及模块 |
|--------|------|---------|
| **假战斗测试** | `get_scene_state` → `enter_game` → `start_new_game` → `set_lobby_state` → `start_run` → `load_scene` → `get_fight_state` → `play_card`/`end_turn` | Gameflow → Lobby → Combat |
| **卡牌 Mod 测试** | `load_scene` → `get_fight_state` → `set_card_pile`(给抽牌堆加测试卡) → `play_card` → `end_turn` → 验证 | Combat |
| **UI 调试** | `raycast_mouse` → `get_screenshot` → `get_scene_tree` | Diagnostics |
